...
@contextmanager
def safe_tempfile(suffix: str = None) -> Generator[str, None, None]:
    """Context manager for safely handling temporary files with auto-cleanup"""
    temp_file = tempfile.NamedTemporaryFile(delete=False, suffix=suffix)
    try:
        yield temp_file.name
    finally:
        try:
            # Ensure file handle is closed before removal
            temp_file.close()
            if os.path.exists(temp_file.name):
                os.remove(temp_file.name)
        except (PermissionError, OSError) as e:
            # Log error but don't raise to prevent API disruption
            print(f"Warning: Failed to cleanup temp file {temp_file.name}: {str(e)}")
