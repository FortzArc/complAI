...
@contextmanager
def safe_tempfile(suffix: Optional[str] = None):
    """Context manager for safely handling temporary files with proper cleanup"""
    temp = tempfile.NamedTemporaryFile(delete=False, suffix=suffix)
    try:
        yield temp.name
    finally:
        try:
            # Ensure we have write permissions before attempting deletion
            os.chmod(temp.name, stat.S_IWRITE | stat.S_IREAD)
            os.unlink(temp.name)
        except (OSError, PermissionError) as e:
            print(f"Warning: Failed to cleanup temp file {temp.name}: {e}")
...
async def upload_pdf(file: UploadFile = File(...)):
    with safe_tempfile(suffix='.pdf') as temp_path:
